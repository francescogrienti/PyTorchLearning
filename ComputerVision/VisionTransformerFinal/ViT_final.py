...
class PatchEmbeddingForMasking(nn.Module):
    """
    Patches embedding module: combination of patches, position and class embeddings.
    Random masking method for the pre-training phase of the Vision Transformer.
    """
...
class PatchEmbeddingForClassification(nn.Module):
    """
    Patches embedding module: combination of patches, position and class embeddings.
    This embedder is used for classification.
    """
...
class MaskedAutoEncoder(nn.Module):
    def __init__(self, embed_size, decoder_embed_size, num_patches, forward_expansion, dropout, num_attention_heads,
        ...
        self.embedding = PatchEmbeddingForMasking(patch_size, embed_size, num_channels, self.image_size, dropout)
...
def pretrain_model(model, optimizer, epochs, mask_ratio):
...
def finetune_model(model, criterion, optimizer, epochs, linear_warmup, cosine_lr):
    train_losses = [0 for _ in range(epochs)]
    test_losses = [0 for _ in range(epochs)]
...
class ViTForClassification(nn.Module):
    ...
    def __init__(self, embed_size, dropout, dataset, num_classes, patch_size, num_channels, encoder):
        super().__init__()
        self.image_size = dataset[0][0].shape[-1]
        self.embed_size = embed_size
        self.num_classes = num_classes
        # Create the embedding layer
        self.embedding = PatchEmbeddingForClassification(patch_size, embed_size, num_channels, self.image_size, dropout)
...
def main():
    pretrained_model = MaskedAutoEncoder(hyper_space["embed_size"], hyper_space["decoder_embed_size"],
                                         ...
                                         hyper_space["forward_expansion"], hyper_space["dropout_rate"],
                                         hyper_space["num_heads"],
                                         hyper_space["qkv_bias"], train_dataset, hyper_space["patch_size"],
                                         hyper_space["encod_hidden_layers"], hyper_space["decoder_hidden_layers"],
                                         hyper_space["num_channels"]).to(device)

    optimizer = optim.AdamW(pretrained_model.parameters(), lr=hyper_space["learning_rate"], weight_decay=1e-2)
    train_losses, test_losses = pretrain_model(pretrained_model, optimizer, hyper_space["epochs"],
                                               hyper_space["mask_ratio"])
    encoder = pretrained_model.encoder
    for param in encoder.parameters():
        param.requires_grad = False  # Se vuoi congelare i pesi inizialmente

    model = ViTForClassification(hyper_space["embed_size"],
                                 hyper_space["dropout_rate"], train_dataset, hyper_space["num_classes"],
                                 hyper_space["patch_size"], hyper_space["num_channels"], encoder).to(device)
    ...
    train_losses, test_losses, train_accuracies, test_accuracies = finetune_model(model, criterion, optimizer,
                                                                                  hyper_space["epochs"],
